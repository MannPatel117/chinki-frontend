...
export class BillingSystemComponent {
        ...
        .then((multi) => {
          if (multi == true) {
            ...
            localStorage.setItem(
              'selectedLocation',
              this.inventorySelected.value
            );
            this.init();
          }
        }) // Will log true or false
        .catch((error) =>
          console.error('Error checking multiInventory:', error)
        );
    }
  }
  ...
  fetchData(location: any) {
    ...
    try {
      this.api
        .getAPI('/inventorys/inventory/' + location, [['details', true]])
        .subscribe((res: any) => {
  ...
  getAllProducts() {
    ...
    try {
      this.api
        .getAPI('/inventoryDetails/billing', [
          ['status', 'active'],
          ['inventory', JSON.stringify([this.inventorySelected.value])],
        ])
        .subscribe((res: any) => {
          this.loading = true;
          if (res.data.length == 0) {
  ...
  setData() {
    this.userForm
      .get('phnNumber')
      ?.setValue(this.currentActiveInvoiceData.UserPhnNumber);
    this.userForm.get('name')?.setValue(this.currentActiveInvoiceData.UserName);
    this.userForm
      .get('address')
      ?.setValue(this.currentActiveInvoiceData.UserAddress);
    this.userForm
      .get('rewardPoints')
      ?.setValue(this.currentActiveInvoiceData.RewardPoints);
    this.userForm
      .get('customerType')
      ?.setValue(this.currentActiveInvoiceData.CustomerType);
    ...
    this.userForm
      .get('paymentType')
      ?.setValue(this.currentActiveInvoiceData.PaymentType);
    setTimeout(() => {
      this.focusBarcode();
      this.loading = false;
    }, 50);
  ...
  scanBarcode() {
    const index = this.allProductsList.findIndex(
      (product: { MasterProduct: any }) =>
        product.MasterProduct.barcode === this.barcodeScan.value
    );
      ...
      ([rowIndex, rowControl]) => {
        ...
        return (
          ...
          itemNameControl.value ===
            this.allProductsList[index].MasterProduct.productID
        );
      }
    );
    ...
    if (existingItemRow) {
        ...
        this.rows
          ...
          ?.setValue(
            this.calcFinalAmount(
              this.allProductsList[index].MasterProduct.sellingPrice,
              this.rows.get(existingRowIndex)?.get('quantity')?.value
            )
          );
        this.currentActiveInvoiceData.BillDetails[rowCount] =
          this.rows.get(existingRowIndex)?.value;
        this.billData.storeData(
          this.currentActiveInvoice,
          this.currentActiveInvoiceData
        );
      }
    } else {
      this.rows
        ...
        ?.get('productName')
        ?.setValue(this.allProductsList[index].MasterProduct.productName);
      this.rows.get(currentRowString)?.get('quantity')?.setValue(1);
      this.rows
        .get(currentRowString)
        ?.get('mrp')
        ?.setValue(this.allProductsList[index].MasterProduct.mrp);
      this.rows
        .get(currentRowString)
        ?.get('barcode')
        ?.setValue(this.allProductsList[index].MasterProduct.barcode);
      this.rows
        .get(currentRowString)
        ?.get('discount')
        ?.setValue(
          this.calcDiscount(
            this.allProductsList[index].MasterProduct.mrp,
            this.allProductsList[index].MasterProduct.sellingPrice
          )
        );
      this.rows
        .get(currentRowString)
        ?.get('rate')
        ?.setValue(this.allProductsList[index].MasterProduct.sellingPrice);
      this.rows
        .get(currentRowString)
        ?.get('amount')
        ?.setValue(
          this.calcAmount(
            this.allProductsList[index].MasterProduct.sellingPrice,
            this.allProductsList[index].MasterProduct.gst
          )
        );
      this.rows
        .get(currentRowString)
        ?.get('gst')
        ?.setValue(this.allProductsList[index].MasterProduct.gst);
      this.rows
        .get(currentRowString)
        ?.get('gstAmount')
        ?.setValue(
      ...
      this.rows
        ...
        ?.setValue(
          this.calcFinalAmount(
            this.allProductsList[index].MasterProduct.sellingPrice,
            this.rows.get(currentRowString)?.get('quantity')?.value
          )
        );
      this.rows
        .get(currentRowString)
        ?.get('productID')
        ?.setValue(this.allProductsList[index].productID);
      this.rows
        .get(currentRowString)
        ?.get('productType')
        ?.setValue(this.allProductsList[index].MasterProduct.productType);
      this.currentActiveInvoiceData.BillDetails[this.currentRow] =
        this.rows.get(currentRowString)?.value;
  ...
  itemChanged(changed: string, currentRow: number) {
    ...
    if (changed == 'quantity') {
      if (this.rows.get(row)?.get('quantity')?.value == 0) {
        ...
        this.rows
          ...
          ?.setValue(
            this.calcFinalAmount(
              this.rows.get(row)?.get('rate')?.value,
              this.rows.get(row)?.get('quantity')?.value
            )
          );
      }
    } else if (changed == 'mrp') {
      ...
      this.rows
        ...
        ?.get('rate')
        ?.setValue(
          this.calcRate(this.rows.get(row)?.get('mrp')?.value, discount)
        );
      this.rows
        .get(row)
        ?.get('amount')
        ?.setValue(
          this.calcAmount(
            this.rows.get(row)?.get('rate')?.value,
            this.rows.get(row)?.get('gst')?.value
          )
        );
      this.rows
        .get(row)
      ...
      this.rows
        ...
        ?.setValue(
          this.calcDiscount(
            this.rows.get(row)?.get('mrp')?.value,
            this.rows.get(row)?.get('rate')?.value
          )
        );
      this.rows
    ...
    this.currentActiveInvoiceData.BillDetails[currentRow] =
      this.rows.get(row)?.value;
    this.billData.storeData(
      this.currentActiveInvoice,
      this.currentActiveInvoiceData
    );
  ...
  calcTotalAmount() {
    ...
    for (let i = 0; i < this.currentRow; i++) {
      let row = String(i);
      this.currentActiveInvoiceData.totalAmount =
        this.currentActiveInvoiceData.totalAmount +
        this.rows.get(row)?.get('finalAmount')?.value;
    }
    if (this.currentActiveInvoiceData.currentOffer.offerID != '') {
      if (
        this.currentActiveInvoiceData.currentOffer.offerType == 'free_product'
      ) {
        if (
          this.currentActiveInvoiceData.totalAmount <
          this.currentActiveInvoiceData.currentOffer.minOrderValue
        ) {
          this.offerUnApply();
        }
      }
      if (
        this.currentActiveInvoiceData.currentOffer.offerType == 'flat_discount'
      ) {
        if (
          this.currentActiveInvoiceData.totalAmount <
          this.currentActiveInvoiceData.currentOffer.minOrderValue -
            this.currentActiveInvoiceData.currentOffer.discountAmount
        ) {
          this.offerUnApply();
        }
      }
  ...
  calcTotalAmountFinished() {
    ...
    for (
      let i = 0;
      i < this.currentActiveInvoiceData.BillFinished.length;
      i++
    ) {
      let row = String(i);
      this.currentActiveInvoiceData.totalAmountF =
        this.currentActiveInvoiceData.totalAmountF +
        this.currentActiveInvoiceData.BillFinished[i].finalAmount;
    }
    this.billData.storeData(
      this.currentActiveInvoice,
      this.currentActiveInvoiceData
  ...
  getMRP(id: string) {
    let product = this.allProductsList.find(
  ...
  customerTypeChanged() {
    this.currentActiveInvoiceData.CustomerType =
      this.userForm.get('customerType')?.value;
    this.billData.storeData(
      this.currentActiveInvoice,
      this.currentActiveInvoiceData
    );
  ...
  applyReedemCode() {
    if (
      this.redeemPoints.value <= this.currentActiveInvoiceData.RewardPoints &&
      this.redeemPoints.value >= 0
    ) {
      this.currentActiveInvoiceData.totalAmount =
        this.currentActiveInvoiceData.totalAmount - this.redeemPoints.value;
  ...
  convertOfferData(offer: any): any {
    return {
      ...
      discountPerc: offer.discountPerc
        ? parseFloat(offer.discountPerc as string)
        : null,
      discountAmount: offer.discountAmount
        ? parseFloat(offer.discountAmount as string)
        : null,
      actualDiscountAmount: offer.actualDiscountAmount
        ? parseFloat(offer.actualDiscountAmount as string)
        : null,
    };
  }
  ...
  offerApplied(offer: any, isCoupon: boolean) {
    ...
    this.currentActiveInvoiceData.currentOffer.minOrderValue =
      this.selectedOffer.minOrderValue;
    ...
    if (this.selectedOffer.offerType == 'free_product') {
      this.currentActiveInvoiceData.currentOffer.FreeProduct.productName =
        this.selectedOffer.FreeProduct.productName;
      this.currentActiveInvoiceData.currentOffer.FreeProduct.productID =
        this.selectedOffer.FreeProduct.productID;
      this.currentActiveInvoiceData.currentOffer.FreeProduct.barcode =
        this.selectedOffer.FreeProduct.barcode;
      this.currentActiveInvoiceData.currentOffer.FreeProduct.mrp = this.getMRP(
        this.selectedOffer.FreeProduct.productID
      );
      this.selectedOffer.FreeProduct.mrp = this.getMRP(
        this.selectedOffer.FreeProduct.productID
      );
    } else if (this.selectedOffer.offerType == 'flat_discount') {
      this.currentActiveInvoiceData.currentOffer.discountPerc =
        this.selectedOffer.discountPerc;
      let amount =
        this.currentActiveInvoiceData.totalAmount *
        (this.selectedOffer.discountPerc / 100);
      let finalAmount = parseFloat(amount.toFixed(2));
      if (amount >= this.selectedOffer.discountAmount) {
        this.currentActiveInvoiceData.currentOffer.actualDiscountAmount =
          this.selectedOffer.discountAmount;
        this.selectedOffer.actualDiscountAmount =
          this.selectedOffer.discountAmount;
      } else {
  ...
  applyCoupon() {
    ...
    try {
      this.api
        .getAPI('/offers/couponRedeem/' + this.couponSearch.value, [])
        .subscribe((res: any) => {
          if (res.success == true) {
            if (res.data.length == 0) {
              ...
              try {
                this.api
                  .getAPI('/offers/offer/' + id, [])
                  .subscribe((res: any) => {
                    if (res.success == true) {
                      this.currentActiveInvoiceData.currentOffer.couponID =
                        this.couponSearch.value;
                      this.currentActiveInvoiceData.currentOffer.isCoupon =
                        true;
  ...
  paymentType() {
    setTimeout(() => {
      this.currentActiveInvoiceData.PaymentType =
        this.userForm.get('paymentType')?.value;
      this.billData.storeData(
        this.currentActiveInvoice,
        this.currentActiveInvoiceData
      );
  ...
  async filterFinished() {
    return new Promise((resolve) => {
      this.currentActiveInvoiceData.BillFinished =
        this.currentActiveInvoiceData.BillDetails.filter(
  ...
  async saveBill(print: boolean) {
    ...
    try {
      this.api.postAPI('/bills/bill', [], body).subscribe(
        (res: any) => {
          this.loading = false;
      ...
      setTimeout(async () => {
        ...
        await this.initializeEmptyTable(rows).then(() => {
          ...
          if (this.currentActiveInvoiceData.BillDetails.length > 0) {
            for (
              let index = 0;
              index < this.currentActiveInvoiceData.currentRow;
              index++
            ) {
              ...
              this.rows
                ...
                ?.get('productName')
                ?.setValue(
                  this.currentActiveInvoiceData.BillDetails[index].productName
                );
              this.rows
                .get(currentRowString)
                ?.get('quantity')
                ?.setValue(
                  this.currentActiveInvoiceData.BillDetails[index].quantity
                );
              this.rows
                .get(currentRowString)
                ?.get('mrp')
                ?.setValue(
                  this.currentActiveInvoiceData.BillDetails[index].mrp
                );
              this.rows
                .get(currentRowString)
                ?.get('discount')
                ?.setValue(
                  this.currentActiveInvoiceData.BillDetails[index].discount
                );
              this.rows
                .get(currentRowString)
                ?.get('rate')
                ?.setValue(
                  this.currentActiveInvoiceData.BillDetails[index].rate
                );
              this.rows
                .get(currentRowString)
                ?.get('amount')
                ?.setValue(
                  this.currentActiveInvoiceData.BillDetails[index].amount
                );
              this.rows
                .get(currentRowString)
                ?.get('gst')
                ?.setValue(
                  this.currentActiveInvoiceData.BillDetails[index].gst
                );
              this.rows
                .get(currentRowString)
                ?.get('gstAmount')
                ?.setValue(
                  this.currentActiveInvoiceData.BillDetails[index].gstAmount
                );
              this.rows
                .get(currentRowString)
                ?.get('finalAmount')
                ?.setValue(
                  this.currentActiveInvoiceData.BillDetails[index].finalAmount
                );
              this.rows
                .get(currentRowString)
                ?.get('productID')
                ?.setValue(
                  this.currentActiveInvoiceData.BillDetails[index].productID
                );
              this.rows
                .get(currentRowString)
                ?.get('productType')
                ?.setValue(
                  this.currentActiveInvoiceData.BillDetails[index].productType
                );
            }
          }
    ...
    return this.searchTerm === ''
      ...
      : this.allProductsList.filter(
          (product: {
            ...
            [
              product.MasterProduct?.productName,
              product.MasterProduct?.barcode,
              product.MasterProduct?.aliasName,
            ].some((field) =>
  ...
  onKeydown(event: KeyboardEvent) {
    ...
    if (event.key === 'ArrowDown') {
      ...
      this.hoverselectedIndex =
        (this.hoverselectedIndex + 1) % this.filteredProduct.length;
    } else if (event.key === 'ArrowUp') {
      event.preventDefault();
      this.hoverselectedIndex =
        (this.hoverselectedIndex - 1 + this.filteredProduct.length) %
        this.filteredProduct.length;
    } else if (event.key === 'Enter') {
      event.preventDefault();
      if (this.hoverselectedIndex >= 0) {
        this.selectProduct(
          this.filteredProduct[this.hoverselectedIndex].MasterProduct.barcode
        );
      }
  ...
  */
  ...
  addUser() {
    if (!this.addUserForm.invalid) {
      ...
      try {
        this.api.postAPI('/users/user', [], this.addUserForm.value).subscribe(
          (res: any) => {
            this.loading = false;
            if (res.success == true) {
              this.toastr.show('success', res.message, {
  ...
  editUser(phoneNumber: any, customerType: any) {
    ...
    this.api
      .putAPI(`/users/user/` + phoneNumber, [], data)
      .subscribe((res: any) => {});
  }
  ...
  clearBill() {
    ...
    this.billDataForm.reset();
    this.currentRow = 0;
    this.currentActiveInvoiceData = this.billData.deleteData(
      this.currentActiveInvoice
    );
    this.fetchData(this.inventorySelected.value);
    setTimeout(() => {
  ...
  getRewardsHistory(phone: any) {
    this.api
      .getAPI('/users/rewards', [['phoneNumber', phone]])
      .subscribe((res: any) => {
        if (res.success == true) {
  ...
  addProduct(index: number, barcode: any) {
    console.log(barcode);
    this.barcodeScan.setValue(barcode);
    this.scanBarcode();
  }
  ...
  getProductQuantity(productID: string): number {
    for (
      ...
      let currentRowString = String(index);
      const row = this.rows.get(currentRowString);

      // Check if productID matches and return quantity
      if (row?.get('productID')?.value === productID) {
        return row?.get('quantity')?.value || 0;
      }
    }
  ...
  getTotalQuantity(): number {
    ...
    for (
      ...
      let currentRowString = String(index);
      const row = this.rows.get(currentRowString);
      quantity = quantity + row?.get('quantity')?.value;
      // Check if productID matches and return quantity
    }
    return quantity || 0; // Return 0 if product not found
  }
  ...
  decreaseQuantity(productID: string) {
    ...
    const index = this.allProductsList.findIndex(
      (product: { MasterProduct: any }) =>
        product.MasterProduct.productID === productIDNUM
    );
    console.log(index);
    const existingItemRow = Object.entries(this.rows.controls).find(
      ([rowIndex, rowControl]) => {
    ...
    if (existingItemRow) {
      ...
      if (quantityControl) {
        const currentQuantity = quantityControl.value || 0;
        let rowIndexNUM = Number(existingRowIndex);
        quantityControl.setValue(currentQuantity - 1);
        this.itemChanged('quantity', rowIndexNUM);
      }
    }
  }
  ...
  this.productForm.patchValue({
    ...
    finalAmount: selectedRow.finalAmount,
    rowIndex: this.editIndex
  });
    $('#bill-editProductModal').modal('show');
  }
